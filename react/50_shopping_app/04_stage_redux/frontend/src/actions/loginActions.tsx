...
const handleLogin = async (request:Request,act:string,dispatch:ThunkDispatch<ApplicationState,null,Action>) => {
	dispatch(loading());
	const response = await fetch(request);
	dispatch(stopLoading());
	if(!response) {
		dispatch(logoutFailed("Server failed to respond. Resetting."));
		return;
	}
	if(response.ok) {
		switch(act) {
